...
- Entities addressed by commit string, test name, run UUID, regression UUID, or parameter key name -- never by internal auto-increment database IDs
...
- Simple offset-based or unpaginated for bounded/small lists: API keys
...
- Supported filter types per endpoint (examples):
  - `test=`, `metric=`, `search=` (case-insensitive substring; see D9)
  ...
  - `commit=` (for regressions)
  - `sort=<field>` (prefix with `-` for descending: `sort=-submitted_at`)
- Run parameter filtering on GET browsing endpoints uses `?param.X=Y` query parameters, where X is the parameter key and Y is the value. Different keys combine with AND; multiple values for the same key combine with OR. Example: `?param.arch=x86_64&param.os=linux&param.os=macos` selects runs where arch is x86_64 AND os is linux or macos.
- On POST query endpoints (`/samples`, `/trends`), run parameters are passed as a `"params"` dict in the JSON request body.
- The `reject_unknown_params` validation function allows the `param.*` prefix pattern in addition to explicitly declared query parameters.
- Exact filters and available sort fields defined per endpoint in the OpenAPI spec
- Filtering by a nonexistent entity name (`test=`) returns 404. Filtering by an unknown metric returns 400.
...
- Standardized error format:
  `{"error": {"code": "not_found", "message": "Test 'foo' not found in test suite 'nts'"}}`
...
- API keys with scopes (each scope includes all scopes above it):
  ...
  - **manage** -- dashboard configuration; update/delete commits; delete runs
